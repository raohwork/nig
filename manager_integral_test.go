...
// managerAbortSuite tests abort behavior in Manager approach
type managerAbortSuite struct {
	suite.Suite
}

func TestManagerIntegral(t *testing.T) {
	suite.Run(t, new(managerAbortSuite))
}

func (s *managerAbortSuite) Test_Normal() {
	...
	type TestArgs struct {
		ValueA int `nig:"dep_a"`
		ValueB int `nig:"dep_b"`
	}
	...
	mgr := New(router)
	mgr.Register("dep_b", depB)
	mgr.Register("dep_a", depA)
	mgr.GET("/test", handler)
...
func (s *managerAbortSuite) Test_AbortInMiddlewareParameter() {
	// Track middleware execution order
	var callOrder []string
	var handlerCalled bool
...
func (s *managerAbortSuite) Test_AbortInCreateFunction() {
	// Track middleware execution order
	var callOrder []string
	var handlerCalled bool
